...
export default function LobbySemaforo({ uid, onEntrar, onVolverAlMenu }) {
  ...
  if (pantalla === "unir") return (
    <div style={{ ...S.page, justifyContent: "flex-start", padding: "36px 20px" }}>
      ...
      <label style={S.fieldLabel} htmlFor="nombre-unir-s">Tu nombre</label>
      <input id="nombre-unir-s" style={{ ...S.input, width: "100%", marginBottom: 12 }} placeholder="Ej. Jorge" value={nombre}
        onChange={(e) => setNombre(e.target.value)} maxLength={12} />
      <label style={S.fieldLabel} htmlFor="codigo-unir-s">Código de la sala</label>
      <input id="codigo-unir-s" style={{ ...S.input, width: "100%", marginBottom: 16, fontSize: 24, letterSpacing: 6, textAlign: "center" }}
